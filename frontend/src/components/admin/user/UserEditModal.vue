    ...
    <form v-if="user" id="edit-user-form" @submit.prevent="handleUpdateUser" class="space-y-5">
      ...
      <div class="space-y-4 rounded-xl border border-gray-200 p-4 dark:border-dark-700">
        <label class="flex items-center justify-between gap-3">
          <span>
            <span class="block text-sm font-medium text-gray-700 dark:text-gray-300">
              {{ t('admin.users.fallback.enabled') }}
            </span>
            <span class="mt-0.5 block text-xs text-gray-500 dark:text-gray-400">
              {{ t('admin.users.fallback.enabledHint') }}
            </span>
          </span>
        ...
        <div v-if="form.subscription_balance_fallback_enabled" class="grid gap-4 sm:grid-cols-2">
          <div>
            <label class="input-label">{{ t('admin.users.fallback.group') }}</label>
            <select v-model.number="form.subscription_balance_fallback_group_id" class="input">
              <option :value="null">{{ t('admin.users.fallback.selectGroup') }}</option>
...
const fallbackGroupOptions = computed(() =>
  groups.value.filter(g => g.status === 'active' && g.subscription_type === 'standard')
)
...
const handleUpdateUser = async () => {
  ...
  if (form.subscription_balance_fallback_enabled) {
    if (!form.subscription_balance_fallback_group_id) {
      appStore.showError(t('admin.users.fallback.groupRequired'))
      return
    }
    if (form.subscription_balance_fallback_limit_usd <= 0) {
      appStore.showError(t('admin.users.fallback.limitRequired'))
      return
    }
  }
