...
export async function getProgress(): Promise<ActiveSubscriptionProduct[]> {
  const { data } = await apiClient.get<ActiveSubscriptionProduct[]>('/subscription-products/progress')
  return data
}
