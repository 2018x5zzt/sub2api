...
type AccountGroup struct {
	...
	// User-facing billing multiplier for this account inside this group
	...
	for i := range columns {
		...
		case accountgroup.FieldBillingMultiplier:
			if value, ok := values[i].(*sql.NullFloat64); !ok {
				return fmt.Errorf("unexpected type %T for field billing_multiplier", values[i])
			} else if value.Valid {
				_m.BillingMultiplier = value.Float64
			}
		case accountgroup.FieldCreatedAt:
...
func (_m *AccountGroup) String() string {
	...
	builder.WriteString(", ")
	builder.WriteString("billing_multiplier=")
	builder.WriteString(fmt.Sprintf("%v", _m.BillingMultiplier))
	builder.WriteString(", ")
