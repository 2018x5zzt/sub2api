...
// The GroupHealthSnapshotFunc type is an adapter to allow the use of ordinary
// function as GroupHealthSnapshot mutator.
type GroupHealthSnapshotFunc func(context.Context, *ent.GroupHealthSnapshotMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f GroupHealthSnapshotFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.GroupHealthSnapshotMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.GroupHealthSnapshotMutation", m)
}
