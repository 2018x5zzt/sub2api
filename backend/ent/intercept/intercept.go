...
// The GroupHealthSnapshotFunc type is an adapter to allow the use of ordinary function as a Querier.
type GroupHealthSnapshotFunc func(context.Context, *ent.GroupHealthSnapshotQuery) (ent.Value, error)

// Query calls f(ctx, q).
func (f GroupHealthSnapshotFunc) Query(ctx context.Context, q ent.Query) (ent.Value, error) {
	if q, ok := q.(*ent.GroupHealthSnapshotQuery); ok {
		return f(ctx, q)
	}
	return nil, fmt.Errorf("unexpected query type %T. expect *ent.GroupHealthSnapshotQuery", q)
}

// The TraverseGroupHealthSnapshot type is an adapter to allow the use of ordinary function as Traverser.
type TraverseGroupHealthSnapshot func(context.Context, *ent.GroupHealthSnapshotQuery) error

// Intercept is a dummy implementation of Intercept that returns the next Querier in the pipeline.
func (f TraverseGroupHealthSnapshot) Intercept(next ent.Querier) ent.Querier {
	return next
}

// Traverse calls f(ctx, q).
func (f TraverseGroupHealthSnapshot) Traverse(ctx context.Context, q ent.Query) error {
	if q, ok := q.(*ent.GroupHealthSnapshotQuery); ok {
		return f(ctx, q)
	}
	return fmt.Errorf("unexpected query type %T. expect *ent.GroupHealthSnapshotQuery", q)
}
...
func NewQuery(q ent.Query) (Query, error) {
	...
	case *ent.GroupHealthSnapshotQuery:
		return &query[*ent.GroupHealthSnapshotQuery, predicate.GroupHealthSnapshot, grouphealthsnapshot.OrderOption]{typ: ent.TypeGroupHealthSnapshot, tq: q}, nil
