-- xlab 产品订阅运行时表（从 xlabapi-snapshot-0.1.137 吸收，干净幂等建表）。
-- 包含：subscription_products / subscription_product_groups / user_product_subscriptions，
-- 以及 usage_logs 的产品结算列与 redeem_codes.product_id。全部 IF NOT EXISTS，fresh install 安全。
