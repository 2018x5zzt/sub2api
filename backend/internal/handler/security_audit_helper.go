...
type securityAuditCompletionToken struct {
	Protocol   string
	Model      string
...
func runSecurityAudit(c *gin.Context, reqLog *zap.Logger, coordinator *securityaudit.Coordinator, legacy *service.ContentModerationService, apiKey *service.APIKey, subject middleware2.AuthSubject, protocol, model string, body []byte, stage string) *securityaudit.Decision {
	...
	completion := newSecurityAuditCompletionToken(request)
	cacheCompletion := cachesSecurityAuditCompletion(stage)
	if cacheCompletion && hasMatchingSecurityAuditCompletion(c, completion) {
		return nil
	}
	if coordinator == nil {
		...
		if decision.AllowNextStage && cacheCompletion {
			c.Set(securityAuditCompletedContextKey, completion)
		}
		return &decision
	}
	if reqLog != nil {
...
func newSecurityAuditCompletionToken(request securityaudit.Request) securityAuditCompletionToken {
	token := securityAuditCompletionToken{
		Protocol:   strings.TrimSpace(request.Protocol),
		Model:      strings.TrimSpace(request.Model),
		BodySHA256: sha256.Sum256(request.Body),
		UserID:     request.UserID,
		APIKeyID:   request.APIKeyID,
	}
	if request.GroupID != nil {
		token.GroupID = *request.GroupID
		token.HasGroup = true
	}
	return token
}

func securityAuditCompletionFromContext(c *gin.Context) (securityAuditCompletionToken, bool) {
	if c == nil {
		return securityAuditCompletionToken{}, false
	}
	value, exists := c.Get(securityAuditCompletedContextKey)
	if !exists {
		return securityAuditCompletionToken{}, false
	}
	token, ok := value.(securityAuditCompletionToken)
	return token, ok
}

func hasMatchingSecurityAuditCompletion(c *gin.Context, expected securityAuditCompletionToken) bool {
	cached, ok := securityAuditCompletionFromContext(c)
	return ok && cached == expected
}

func cacheSecurityAuditCompletion(c *gin.Context, apiKey *service.APIKey, subject middleware2.AuthSubject, protocol, model string, body []byte) {
	if c == nil || c.Request == nil {
		return
	}
	request := buildSecurityAuditRequest(c, apiKey, subject, protocol, model, body, "http")
	c.Set(securityAuditCompletedContextKey, newSecurityAuditCompletionToken(request))
}
