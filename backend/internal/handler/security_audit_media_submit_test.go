...
func TestAsyncImageSuccessfulPrecheckIsNotRepeatedByDetachedExecution(t *testing.T) {
	...
	h.execute = func(_ string, c *gin.Context) {
		...
		decision := openAI.checkSecurityAudit(c, nil, apiKey, subject, service.ContentModerationProtocolOpenAIImages, "gpt-image-2", []byte(`{"prompt":"allowed async prompt"}`))
		executionMu.Lock()
...
func TestOpenAIImageJobPromptGuardRunsBeforePersistence(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	router := gin.New()
	router.Use(securityAuditMediaTestMiddleware)
	router.POST("/v1/images/jobs/generations", h.ImageJobCreate)
	request := httptest.NewRequest(http.MethodPost, "/v1/images/jobs/generations", strings.NewReader(`{"model":"gpt-image-2","prompt":"blocked job prompt"}`))
	request.Header.Set("Content-Type", "application/json")
	recorder := httptest.NewRecorder()
	router.ServeHTTP(recorder, request)
	...
	require.Contains(t, recorder.Body.String(), securityaudit.ErrorCodeBlocked)
	persistence.mu.Lock()
	require.Empty(t, persistence.jobs, "no image job may be persisted after a blocking decision")
	persistence.mu.Unlock()
	evaluated, _, requests := engine.snapshot()
	require.Equal(t, 1, evaluated)
	require.Len(t, requests, 1)
	require.Contains(t, string(requests[0].Body), "blocked job prompt")
}

func TestOpenAIImageJobSuccessfulPrecheckIsNotRepeatedByDetachedExecution(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	c.Request = httptest.NewRequest(http.MethodPost, "/v1/images/jobs/generations", strings.NewReader(`{"model":"gpt-image-2","prompt":"allowed job prompt"}`))
	decision := h.checkSecurityAudit(c, nil, apiKey, subject, service.ContentModerationProtocolOpenAIImages, "gpt-image-2", []byte(`{"prompt":"allowed job prompt"}`))
	require.NotNil(t, decision)
	require.True(t, decision.AllowNextStage)

	securityAuditCompletion, hasSecurityAuditCompletion := securityAuditCompletionFromContext(c)
	detachedReq := openAIImageJobRequest{
		Endpoint:                   EndpointImagesGenerations,
		ContentType:                "application/json",
		Headers:                    make(http.Header),
		Body:                       []byte(`{"model":"gpt-image-2","prompt":"allowed job prompt"}`),
		SecurityAuditCompletion:    securityAuditCompletion,
		HasSecurityAuditCompletion: hasSecurityAuditCompletion,
	}.clone()
	detachedRecorder := httptest.NewRecorder()
	detachedCtx, _ := gin.CreateTestContext(detachedRecorder)
	detachedCtx.Request = httptest.NewRequest(http.MethodPost, detachedReq.Endpoint, strings.NewReader(string(detachedReq.Body)))
	applyOpenAIImageJobSecurityAuditCompletion(detachedCtx, detachedReq)
	repeatedDecision := h.checkSecurityAudit(detachedCtx, nil, apiKey, subject, service.ContentModerationProtocolOpenAIImages, "gpt-image-2", []byte(`{"prompt":"allowed job prompt"}`))

	require.Nil(t, repeatedDecision)
	evaluated, _, _ := engine.snapshot()
	require.Equal(t, 1, evaluated, "detached execution must reuse the successful submit-stage audit")
}
