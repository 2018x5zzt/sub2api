...
func (h *OpenAIGatewayHandler) Images(c *gin.Context) {
	...
	for {
		...
		// fork: acquireResponsesAccountSlot 增加了 allowQueueWait 参数与 waitSkipped 返回(429 failover)；
		// images 路径无 429-failover 循环，允许排队等待，忽略 waitSkipped。
		accountReleaseFunc, acquired, _ := h.acquireResponsesAccountSlot(c, apiKey.GroupID, sessionHash, selection, parsed.Stream, &streamStarted, true, reqLog)
		if !acquired {
			return
		}
