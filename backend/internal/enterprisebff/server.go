...
type Server struct {
	...
	healthRepo      groupHealthSnapshotReader
}

func New(cfg *Config, adminKeySvc AdminKeyStore, enterpriseStore EnterpriseStore, healthRepo groupHealthSnapshotReader) *Server {
	s := &Server{
		cfg: cfg,
		httpClient: &http.Client{
...
func (s *Server) newRouter() *gin.Engine {
	...
	r.POST("/auth/forgot-password", func(c *gin.Context) { s.proxy(c, "/auth/forgot-password", nil) })
	r.POST("/auth/reset-password", func(c *gin.Context) { s.proxy(c, "/auth/reset-password", nil) })
	...
	r.POST("/auth/revoke-all-sessions", func(c *gin.Context) { s.proxy(c, "/auth/revoke-all-sessions", nil) })
	r.GET("/auth/me", s.handleEnterpriseMe)
	r.GET("/settings/public", s.handleEnterprisePublicSettings)
	r.GET("/user/profile", func(c *gin.Context) { s.proxy(c, "/user/profile", nil) })
	r.PUT("/user", func(c *gin.Context) { s.proxy(c, "/user", nil) })
	r.PUT("/user/password", func(c *gin.Context) { s.proxy(c, "/user/password", nil) })
	r.GET("/user/totp/status", func(c *gin.Context) { s.proxy(c, "/user/totp/status", nil) })
	r.GET("/groups/available", s.handleEnterpriseVisibleGroups)
	r.GET("/groups/pool-status", s.handleEnterprisePoolStatus)
	r.GET("/subscriptions/summary", func(c *gin.Context) { s.proxy(c, "/subscriptions/summary", nil) })
	r.GET("/subscriptions/progress", func(c *gin.Context) { s.proxy(c, "/subscriptions/progress", nil) })
	r.GET("/redeem/history", func(c *gin.Context) { s.proxy(c, "/redeem/history", nil) })
	r.POST("/redeem", func(c *gin.Context) { s.proxy(c, "/redeem", nil) })
