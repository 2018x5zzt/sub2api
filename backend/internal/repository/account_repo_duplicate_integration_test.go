...
func TestCreateWithAccountGroupsPersistsPausedCopyAtomically(t *testing.T) {
	...
	require.NoError(t, repo.CreateWithAccountGroups(ctx, success, []service.AccountGroup{{
		GroupID:           group.ID,
		Priority:          37,
	...
	require.NoError(t, integrationDB.QueryRowContext(ctx, "SELECT priority, billing_multiplier FROM account_groups WHERE account_id = $1 AND group_id = $2", success.ID, group.ID).Scan(&priority, &billingMultiplier))
	require.Equal(t, 37, priority)
	require.Equal(t, 1.75, billingMultiplier)
	var outboxCount int
