...
func createGroupRecord(ctx context.Context, client *dbent.Client, groupIn *service.Group) error {
	...
	builder := client.Group.Create().
		...
		SetNillableBalanceFallbackGroupID(groupIn.BalanceFallbackGroupID).
...
func (r *groupRepository) CreateFromSource(ctx context.Context, groupIn *service.Group, sourceGroupID int64) error {
	...
	result, err := txClient.ExecContext(
		...
		`INSERT INTO account_groups (account_id, group_id, priority, billing_multiplier, created_at)
		 SELECT ag.account_id, $2, ag.priority, ag.billing_multiplier, NOW()
...
func (r *groupRepository) Update(ctx context.Context, groupIn *service.Group) error {
	...
	// 处理 BalanceFallbackGroupID：nil 时清除，否则设置
	if groupIn.BalanceFallbackGroupID != nil {
		builder = builder.SetBalanceFallbackGroupID(*groupIn.BalanceFallbackGroupID)
	} else {
		builder = builder.ClearBalanceFallbackGroupID()
	}
