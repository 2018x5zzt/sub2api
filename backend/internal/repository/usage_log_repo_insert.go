...
const usageLogInsertColumnList = `
			user_id,
			api_key_id,
			account_id,
			request_id,
...
func buildUsageLogDirectInsertQuery(returning bool) string {
	...
	return fmt.Sprintf(`
		INSERT INTO usage_logs (%s
		) VALUES (
			%s
		)
		ON CONFLICT (request_id, api_key_id) DO NOTHING%s
	`, usageLogInsertColumnList, strings.Join(placeholders, ", "), returningClause)
}
...
func (r *usageLogRepository) createSingle(ctx context.Context, sqlq sqlExecutor, log *service.UsageLog) (bool, error) {
	...
	query := buildUsageLogDirectInsertQuery(true)
...
func execUsageLogInsertNoResult(ctx context.Context, sqlq sqlExecutor, prepared usageLogInsertPrepared) error {
	query := buildUsageLogDirectInsertQuery(false)
...
func prepareUsageLogInsert(log *service.UsageLog) usageLogInsertPrepared {
	...
	return usageLogInsertPrepared{
		...
		args: []any{
			...
			log.ProductSubscriptionID, // product_subscription_id
			log.ProductDebitCost,      // product_debit_cost
		},
	}
}
