...
func (s *GatewayService) recordUsageCore(ctx context.Context, input *recordUsageCoreInput, opts *recordUsageOpts) error {
	...
	// fork: 叠加账号分组计费倍率（group billing multiplier），保证分组维度计费倍率在统一计费路径生效
	multiplier *= account.GroupBillingMultiplier(apiKey.GroupID)
	// 上游: token 倍率叠加高峰因子（高峰因子按请求时刻现算，不并入上面的 getUserGroupRateMultiplier，
	// 以免污染 user:group 倍率缓存）；imageMultiplier 经 computePeakAwareMultipliers 内部的
	// resolveImageRateMultiplier 计算，保留 fork 的独立图片费率逻辑。
