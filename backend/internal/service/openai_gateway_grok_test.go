...
func TestBuildGrokResponsesRequestAllowsPublicAPIKeyBaseURLByDefault(t *testing.T) {
	...
	req, err := buildGrokResponsesRequest(context.Background(), nil, account, []byte(`{"model":"grok-4.3"}`), "api-key", "isolated-cache-id", nil)
	require.NoError(t, err)
...
func TestBuildGrokResponsesRequestAPIKeyAllowsCustomBaseURL(t *testing.T) {
	...
	req, err := buildGrokResponsesRequest(context.Background(), nil, account, []byte(`{"model":"grok-4.3"}`), "xai-key", "isolated-cache-id", &config.Config{})
	require.NoError(t, err)
