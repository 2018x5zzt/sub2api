...
type billingMultiplierResolution struct {
	BaseRateMultiplier          float64
	BillingAdjustmentMultiplier float64
	EffectiveBillingMultiplier  float64
	PricingMode                 string
}
...
func resolveBillingMultiplierForUsage(ctx context.Context, apiKey *APIKey, user *User, account *Account, defaultRateMultiplier float64, resolveUserGroupRate userGroupRateResolverFunc) billingMultiplierResolution {
	resolution := billingMultiplierResolution{
		BaseRateMultiplier:          defaultRateMultiplier,
		BillingAdjustmentMultiplier: 1,
		EffectiveBillingMultiplier:  defaultRateMultiplier,
	...
	group := apiKey.Group
	resolution.PricingMode = normalizeGroupPricingMode(group.PricingMode)
	if group.IsDynamicPricing() {
		resolution.BaseRateMultiplier = 1
		resolution.BillingAdjustmentMultiplier = 1
		if account != nil {
			resolution.BillingAdjustmentMultiplier = account.GroupBillingMultiplier(apiKey.GroupID)
		}
		resolution.EffectiveBillingMultiplier = resolution.BaseRateMultiplier * resolution.BillingAdjustmentMultiplier
		return resolution
	}
...
func resolveDynamicPricingBaseMultiplier(ctx context.Context, groupID *int64, resolveUserGroupRate userGroupRateResolverFunc) (float64, bool) {
	group := dynamicPricingGroupFromContext(ctx, groupID)
	if group == nil || !group.IsDynamicPricing() || groupID == nil {
		return 0, false
	}
	return 1, true
}
...
func compareDynamicPricingAccountPreference(ctx context.Context, groupID *int64, a, b *Account, resolveUserGroupRate userGroupRateResolverFunc) int {
	...
	if absFloat(aMultiplier-bMultiplier) <= dynamicBudgetEpsilon {
		return 0
	}
	if aWithinBudget {
...
func isAccountWithinDynamicBudget(ctx context.Context, groupID *int64, account *Account, resolveUserGroupRate userGroupRateResolverFunc) bool {
	...
	if effectiveMultiplier <= budgetMultiplier+dynamicBudgetEpsilon {
		return true
	}
	...
	return state.currentAverageMultiplier < budgetMultiplier
}
