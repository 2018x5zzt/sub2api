...
func (s *PaymentService) ensurePaymentSubscriptionAssigned(ctx context.Context, o *dbent.PaymentOrder, groupID int64, days int) error {
	...
	var legacyCacheService *SubscriptionService
	if !alreadyAssigned {
		orderNote := paymentSubscriptionOrderNote(o.ID)
		recoveredFromNote, legacyCacheService, err = s.findCommittedPaymentSubscription(ctx, o.UserID, groupID, orderNote)
		if err != nil {
			return fmt.Errorf("check existing subscription assignment: %w", err)
		}
		if !recoveredFromNote {
			...
			if legacyAssigner, ok := s.subscriptionAssigner.(*SubscriptionService); ok {
				legacyCacheService = legacyAssigner
				_, _, err = legacyAssigner.assignOrExtendSubscription(txCtx, input, true)
			} else {
				_, _, err = s.subscriptionAssigner.AssignOrExtendSubscription(txCtx, input)
			}
			if err != nil {
		...
		if _, err := txClient.PaymentAuditLog.Create().
			...
			if dbent.IsConstraintError(err) {
				...
				if checkErr == nil && claimed {
					if legacyCacheService != nil {
						return legacyCacheService.invalidateSubscriptionCaches(o.UserID, groupID)
					}
					return nil
				}
			}
	...
	if legacyCacheService != nil {
		// Legacy cache invalidation must happen after the outer transaction commits.
		if err := legacyCacheService.invalidateSubscriptionCaches(o.UserID, groupID); err != nil {
...
func (s *PaymentService) findCommittedPaymentSubscription(ctx context.Context, userID, groupID int64, orderNote string) (bool, *SubscriptionService, error) {
	if productAssigner, ok := s.subscriptionAssigner.(*ProductAwareSubscriptionAssigner); ok && productAssigner.product != nil {
		productID, found, err := productAssigner.ResolveActiveProductIDByGroupID(ctx, groupID)
		if err != nil {
			return false, nil, err
		}
		if found {
			products, err := productAssigner.product.ListActiveUserProducts(ctx, userID)
			if err != nil {
				return false, nil, err
			}
			for i := range products {
				if products[i].Product.ID == productID && hasPaymentSubscriptionOrderNote(products[i].Subscription.Notes, orderNote) {
					return true, nil, nil
				}
			}
		}
	...
	legacyService, _ := s.subscriptionAssigner.(*SubscriptionService)
	if legacyService == nil && s.subscriptionAssigner == nil {
		// Historical recovery only. A missing assigner must never create a new
		// legacy subscription, but an already committed order can be claimed.
		legacyService = s.subscriptionSvc
	}
	if legacyService == nil || legacyService.userSubRepo == nil {
		return false, nil, nil
	}
	existing, err := legacyService.userSubRepo.GetByUserIDAndGroupID(ctx, userID, groupID)
	if err != nil {
	...
	if existing != nil && hasPaymentSubscriptionOrderNote(existing.Notes, orderNote) {
		return true, legacyService, nil
	}
	return false, nil, nil
}
