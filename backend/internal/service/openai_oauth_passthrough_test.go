...
func TestOpenAIGatewayService_OAuthPassthrough_EmptyHTTPResponseTriggersFailover(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	tests := []struct {
		...
		{name: "nil response"},
		{name: "nil response body", resp: &http.Response{StatusCode: http.StatusOK}},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			svc := &OpenAIGatewayService{
				cfg:          &config.Config{Gateway: config.GatewayConfig{ForceCodexCLI: false}},
				httpUpstream: &httpUpstreamRecorder{resp: tt.resp},
			}
			account := &Account{
				ID:          129,
				Name:        "empty-response",
			...
			result, err := svc.Forward(context.Background(), c, account, []byte(`{"model":"gpt-5.2","instructions":"test","input":"hi"}`))
