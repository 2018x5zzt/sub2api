...
export function buildEmbeddedUrl(
  ...
  try {
    ...
    if (userId) url.searchParams.set(EMBEDDED_USER_ID_QUERY_KEY, String(userId))
    if (authToken) url.searchParams.set(EMBEDDED_AUTH_TOKEN_QUERY_KEY, authToken)
    url.searchParams.set(EMBEDDED_THEME_QUERY_KEY, theme)
    if (lang) url.searchParams.set(EMBEDDED_LANG_QUERY_KEY, lang)
