export default {
  ...
  v2Common: {
    ...
    randomPool: '随机池',
    usedMax: '已用 / 上限',
    inputMtok: '输入 / 百万 Token',
    outputMtok: '输出 / 百万 Token',
    userId: '用户 ID',
    apiKeyId: 'API 密钥 ID',
    groupId: '分组 ID',
    platformType: '平台 / 类型',
    ...
    clearError: '清除错误',
    clearRateLimit: '清除限流',
    autoPauseExpired: '过期后自动暂停',
    schedulable: '参与调度',
    schedulableHint: '可用于请求路由',
    visibleAdminsOnly: '仅管理员可见',
    noProxyHint: '留空表示不使用代理',
    higherPreferredHint: '数值越高越优先',
    blankUseGroupHint: '留空表示使用分组配置',
    commaSeparatedGroupIds: '用英文逗号分隔，例如 1,2,5',
    accountExpiryHint: '超过此时间后账号将拒绝请求',
    maskedSecretHint: '已有密钥会显示为脱敏占位符。如需轮换，请输入新值。',
    suggestedCredentialShape: '{platform}/{type} 的建议格式，请替换为真实值。',
    platformCredentialJson: '请以 JSON 填写对应平台的凭据字段。',
    allStatus: '全部状态',
    allPlatforms: '全部平台',
    allScenes: '全部场景',
    allTypes: '全部类型',
    ...
    today: '{date}（今天）',
    tomorrow: '{date}（明天）',
    daysDate: '{days} 天 · {date}',
    currentExpiry: '当前到期',
    noExpirationSymbol: '∞',
    stream: '流式',
    sync: '同步',
    requestType: '请求类型',
    copiedCount: '已复制 {count} 个兑换码',
    generatedCount: '已生成 {count} 个兑换码',
    deletedCount: '已删除 {count} 个兑换码',
    deleteSelected: '删除已选（{count}）',
    deleteCountConfirm: '确定删除 {count} 个兑换码？',
    valuePositive: '面值必须大于 0',
    countRange: '数量必须在 1 到 1000 之间',
    subscriptionGroupRequired: '订阅兑换码需要填写分组 ID',
    fieldRequired: '请填写{field}',
    invalidJson: '凭据必须是有效 JSON：{message}',
    notLoaded: '尚未加载',
    noneSelected: '未选择项目',
    nonZeroDays: '天数不能为 0',
    negativeShortensHint: '负数会缩短有效期，例如 30 表示增加一个月',
    unlimitedValidityHint: '留空或 0 表示无限期',
    editDeferred: '编辑功能尚未迁移到 v2',
    configured: '已配置',
    notSet: '未设置',
    leaveBlankKeepExisting: '留空则保留现有值',
    enterSecret: '输入密钥',
    show: '显示',
    hide: '隐藏',
    testConnection: '测试连接',
    connected: '已连接',
    testFailed: '{target} 测试失败'
  },

  v2Admin: {
    usage: {
      title: '使用记录',
      description: '查看所有用户的请求记录，并支持搜索筛选。'
    },
    subscriptions: {
      title: '订阅管理',
      description: '为用户分配分组订阅，并按需延长或撤销。',
      assign: '分配',
      extend: '延长',
      revoke: '撤销',
      assignTitle: '分配订阅',
      extendTitle: '延长订阅 #{id}',
      searchPlaceholder: '用户邮箱 / 名称...',
      revokeConfirm: '确定撤销订阅 #{id}（{group}）？'
    },
    accounts: {
      description: '管理上游平台账号和平台专用凭据。',
      addTitle: '添加账号',
      editTitle: '编辑 {name}',
      deleteConfirm: '确定删除账号“{name}”吗？'
    },
    groups: {
      description: '管理面向用户的模型分组、计费倍率和限制',
      subscriptionType: '订阅类型',
      rateMultiplier: '费率倍数',
      exclusive: '专属',
      dailyLimit: '每日 $',
    ...
    announcements: {
      description: '发布系统公告，支持定时与登录弹窗。',
      deleteConfirm: '确定删除公告“{title}”吗？'
    },
    promoCodes: {
      title: '优惠码',
      description: '用于注册赠送或福利码发放，支持随机池。',
      createTitle: '创建优惠码',
      editTitle: '编辑 {code}',
      usagesTitle: '使用记录 — {code}',
      deleteConfirm: '确定删除优惠码“{code}”吗？',
      usageTitle: '使用记录'
    },
    redeemCodes: {
      title: '兑换码',
      description: '生成并管理余额、并发或订阅的一次性兑换码。',
      generateTitle: '生成兑换码',
      generatedCount: '已生成 {count} 个兑换码',
      deleteConfirm: '确定删除此兑换码吗？',
      codeCount: '{count} 个兑换码',
      copyAll: '全部复制'
    },
    placeholder: {
      message: '此管理模块尚未迁移到 v2，请查看 {file}。'
    }
  },
  ...
  landing: {
    ...
    operations: {
      ...
      features: {
        routing: {
          title: '智能路由',
          description: '根据延迟、成本与可用性自动选择最佳上游。'
        },
        unified: {
          title: '统一 API 层',
          description: '所有模型遵循同一 OpenAI 兼容接口。'
        },
        reliability: {
          title: '生产级可靠性',
          description: '多账号故障切换、限流处理和运营可观测性。'
        }
      },
      metrics: {
  ...
  keys: {
    ...
    budgetMultiplierHint: '动态倍率分组必填，范围 {min}x - {max}x。当前分组默认值为 {default}x。',
    budgetMultiplierRequired: '请设置预算倍率',
    statusLabel: '状态',
    selectStatus: '选择状态',
    saving: '保存中...',
    noKeysYet: '暂无 API 密钥',
  ...
  redeem: {
    ...
    luckyAmount: '手气红包',
    totalAmount: '总到账',
    viewLuckyLeaderboard: '查看手气榜',
    luckyLeaderboardTitle: '手气排行榜',
    luckyLeaderboardSummary: '已领取的红包按手气金额从高到低排序。',
    randomPoolAmount: '随机红包池',
    remainingRandomPoolAmount: '剩余随机池',
    leaderboardClaimProgress: '已领取 {used} / {total}',
    yourLuckyRank: '你的当前排名第 {rank} 名，手气红包 {amount}，总到账 {total}',
    you: '你',
    usernameRequiredTitle: '先设置用户名',
    usernameRequiredSubtitle: '红包活动需要用户名参与榜单展示',
    usernameRequiredBody: '你当前还没有设置用户名。为了避免手气榜出现匿名记录，请先到个人设置里填写用户名，再回来领取红包。',
    goSetUsername: '去设置用户名'
  },
  ...
  admin: {
    ...
    accounts: {
      ...
      groupBindingMultiplierShort: '绑',
      groupBindingMultiplierValue: '绑定乘数 {rate}x',
      columns: {
        name: '名称',
        platformType: '平台/类型',
        platform: '平台',
    ...
    settings: {
      ...
      site: {
        ...
        docUrl: '文档链接',
        docUrlHint: '文档网站的链接。留空则使用默认 Xlabapi 使用文档。',
