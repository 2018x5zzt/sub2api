...
export default function AdminUsersPage() {
  ...
  return (
    <>
      <PageHeader title={t('nav.users')} description={t('admin.users.description') as string} />
      <Card className="p-4">
            ...
            <TBody>
              {(data?.items ?? []).map((u) => (
                <TR key={u.id}>
                  ...
                  <TD>
                    {u.role === 'admin' ? (
                      <Badge tone="accent">{t('admin.users.roles.admin')}</Badge>
                    ) : (
                      <Badge>{t('admin.users.roles.user')}</Badge>
                    )}
                  </TD>
                  <TD>
