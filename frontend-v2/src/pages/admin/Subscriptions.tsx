...
export default function AdminSubscriptionsPage() {
  ...
  return (
    <>
      <PageHeader
        title="Subscriptions"
        description="Assign group subscriptions to users; extend or revoke as needed."
        actions={
      ...
      <Card className="p-4">
        <div className="flex flex-wrap gap-3 items-center mb-4">
          ...
          >
            ...
            <option value="revoked" className="bg-bg-4">revoked</option>
          </select>
        </div>
                      ...
                      <div className="space-y-1.5">
                        ...
                        {!s.group?.daily_limit_usd &&
                          ...
                          !s.group?.monthly_limit_usd && (
                            <span className="text-xs text-ink-4 font-mono">no limits</span>
                          )}
                      </div>
                    </TD>
                      ...
                      <div className="inline-flex gap-1">
                        ...
                        <button
                          ...
                          onClick={() => {
                            if (
                              confirm(
                                `Revoke subscription #${s.id} (${s.group?.name || s.group_id})?`
                              )
                            ) {
      ...
      >
        <form id="assign-form" onSubmit={onAssignSubmit} className="space-y-4">
          ...
          <Input
            ...
            min="0"
            label="Validity (days)"
            hint="Leave blank or 0 = unlimited"
      ...
      <Modal
        ...
        title={extending ? `Extend subscription #${extending.id}` : ''}
        footer={
          <>
          ...
          {extending && (
            <div className="rounded-lg border border-line-2 bg-bg-2 p-3 text-sm text-ink-2">
              ...
              <div className="text-xs text-ink-3 font-mono mt-1">
                {extending.group?.name || `Group #${extending.group_id}`} · current expiry:{' '}
                {extending.expires_at ? new Date(extending.expires_at).toLocaleDateString() : '∞'}
              </div>
            </div>
          )}
          <Input
            ...
            type="number"
            label="Days to add"
            hint="Negative values shorten; e.g. 30 to add a month"
            value={extendDays}
