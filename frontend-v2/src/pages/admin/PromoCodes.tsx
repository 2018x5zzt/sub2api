...
export default function AdminPromoCodesPage() {
  ...
  return (
    <>
      <PageHeader
        title={t('admin.benefit.title')}
        description={t('admin.benefit.description') as string}
        actions={
      ...
      <Card className="p-4">
        <div className="flex flex-wrap gap-3 items-center mb-4">
          <Input
            name="search"
            placeholder={t('admin.benefit.searchCodes') as string}
            value={search}
          ...
          >
            <option value="" className="bg-bg-4">{t('admin.redeem.allTypes')}</option>
            {SCENES.map((s) => (
          ...
          <select
            ...
            className={selectClass + ' max-w-[160px]'}
          >
            <option value="" className="bg-bg-4">{t('admin.redeem.allStatus')}</option>
            <option value="active" className="bg-bg-4">{t('admin.benefit.statusActive')}</option>
            <option value="disabled" className="bg-bg-4">{t('admin.benefit.statusDisabled')}</option>
          </select>
        </div>
          ...
          <Table>
            <THead>
              <TR>
                ...
                <TH className="text-right">{t('admin.benefit.fixedAmount')}</TH>
                <TH className="text-right">{t('admin.benefit.randomAmount')}</TH>
                <TH className="text-right">{t('admin.benefit.maxUses')}</TH>
                <TH>{t('common.status')}</TH>
                <TH>{t('admin.benefit.expiresAt')}</TH>
                ...
                <TR key={p.id}>
                  ...
                  <TD className="text-right">
                    <div className="inline-flex gap-1">
                      <button
                        title={t('admin.benefit.viewUsages') as string}
      ...
      <Modal
        ...
        onClose={close}
        title={editing ? t('admin.benefit.editCode') : t('admin.benefit.createCode')}
        size="lg"
        footer={
          <>
      ...
      >
        <form id="promo-form" onSubmit={onSubmit} className="space-y-4">
          <div className="grid grid-cols-2 gap-3">
            <Input
              name="code"
              label={t('admin.benefit.code') as string}
              placeholder={t('admin.benefit.autoGenerate') as string}
              value={form.code}
            ...
            />
            <div>
              <label className="input-label">{t('v2Common.scene')}</label>
              <select
                value={form.scene}
          ...
          <div className="grid grid-cols-2 gap-3">
            <Input
              ...
              step="0.01"
              min="0"
              label={t('admin.benefit.fixedAmount') as string}
            ...
            <Input
              ...
              step="0.01"
              min="0"
              label={t('admin.benefit.randomPoolAmount') as string}
              hint={t('admin.benefit.randomAmount') as string}
          ...
          <div className="grid grid-cols-2 gap-3">
            <Input
              ...
              type="number"
              min="0"
              label={t('admin.benefit.maxUses') as string}
              hint={t('admin.benefit.zeroUnlimited') as string}
              value={form.max_uses}
            ...
            <Input
              name="expires_at"
              type="datetime-local"
              label={t('admin.benefit.expiresAt') as string}
          ...
          <label className="inline-flex items-center gap-2 cursor-pointer">
            ...
            />
            <span className="text-sm text-ink-2">{t('admin.benefit.leaderboardEnabled')}</span>
          </label>
          <Input
            name="success_message"
            label={t('admin.benefit.successMessage') as string}
            placeholder={t('admin.benefit.successMessagePlaceholder') as string}
            value={form.success_message}
            onChange={(e) => setForm({ ...form, success_message: e.target.value })}
          />
          <Input
            name="notes"
            label={t('admin.benefit.notes') as string}
            value={form.notes}
      ...
      <Modal
        ...
        title={usagesFor ? t('v2Admin.promoCodes.usagesTitle', { code: usagesFor.code }) : ''}
        size="lg"
        footer={
