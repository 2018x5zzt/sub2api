  ...
  return (
    <>
      <PageHeader
        ...
        description={t('v2Admin.accounts.description') as string}
        actions={
      ...
      <Card className="p-4">
        <div className="flex flex-wrap gap-3 items-center mb-4">
          ...
          >
            <option value="" className="bg-bg-4">{t('v2Common.allPlatforms')}</option>
            {PLATFORMS.map((p) => (
          ...
          >
            ...
            <option value="active" className="bg-bg-4">{t('common.active')}</option>
            <option value="inactive" className="bg-bg-4">{t('common.inactive')}</option>
            <option value="error" className="bg-bg-4">{t('common.error')}</option>
          </select>
        </div>
        ...
        {isLoading ? (
          ...
          <Table>
            <THead>
              <TR>
                <TH>{t('common.name')}</TH>
                <TH>{t('v2Common.platformType')}</TH>
                <TH>{t('common.status')}</TH>
                <TH className="text-right">{t('v2Common.concurrency')}</TH>
                <TH className="text-right">{t('v2Common.priority')}</TH>
                <TH>{t('v2Common.lastUsed')}</TH>
                ...
                <TR key={a.id}>
                  ...
                  <TD className="text-right">
                    <div className="inline-flex gap-1">
                      <button
                        title={t('v2Common.test') as string}
                      ...
                      {(a.type === 'oauth' || a.type === 'setup-token') && (
                        <button
                          title={t('v2Common.refreshCredentials') as string}
                          className="btn btn-ghost btn-icon btn-sm"
                      ...
                      <button
                        title={t(a.schedulable ? 'v2Common.pause' : 'v2Common.resume') as string}
                        className="btn btn-ghost btn-icon btn-sm"
                      ...
                      {a.error_message && (
                        <button
                          title={t('v2Common.clearError') as string}
                      ...
                      {a.rate_limit_reset_at && (
                        <button
                          title={t('v2Common.clearRateLimit') as string}
                          className="btn btn-ghost btn-icon btn-sm"
                      ...
                      <button
                        ...
                        onClick={() => {
                          if (confirm(t('v2Admin.accounts.deleteConfirm', { name: a.name }) as string)) {
      ...
      <Modal
        ...
        onClose={close}
        title={editing ? t('v2Admin.accounts.editTitle', { name: editing.name }) : t('v2Admin.accounts.addTitle')}
        size="lg"
        footer={
          <>
      ...
      >
        <form id="account-form" onSubmit={onSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            ...
            <Input
              name="notes"
              label={t('v2Common.notes') as string}
              hint={t('v2Common.visibleAdminsOnly') as string}
              value={form.notes}
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="input-label">{t('v2Common.platform')}</label>
              <select
                value={form.platform}
            ...
            <div>
              <label className="input-label">{t('v2Common.type')}</label>
              <select
                value={form.type}
          ...
          <div>
            <label className="input-label">{t('v2Common.credentialsJson')}</label>
            <textarea
            ...
            <p className="text-xs text-ink-3 mt-1">
              {editing
                ? t('v2Common.maskedSecretHint')
                : credsHint(form.platform, form.type)
                ? t('v2Common.suggestedCredentialShape', { platform: form.platform, type: form.type })
                : t('v2Common.platformCredentialJson')}
            </p>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            <Input
              ...
              type="number"
              min="0"
              label={t('v2Common.proxyId') as string}
              hint={t('v2Common.noProxyHint') as string}
              value={form.proxy_id}
            ...
            <Input
              ...
              type="number"
              min="0"
              label={t('v2Common.concurrency') as string}
            ...
            <Input
              name="priority"
              type="number"
              label={t('v2Common.priority') as string}
              hint={t('v2Common.higherPreferredHint') as string}
              value={form.priority}
            ...
            <Input
              ...
              min="0"
              label="Rate ×"
              hint={t('v2Common.blankUseGroupHint') as string}
          ...
          <Input
            name="group_ids"
            label={t('v2Common.groupIds') as string}
            hint={t('v2Common.commaSeparatedGroupIds') as string}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 items-end">
            <Input
              name="expires_at"
              type="datetime-local"
              label={t('v2Common.expires') as string}
              hint={t('v2Common.accountExpiryHint') as string}
            ...
            <label className="inline-flex items-center gap-2 cursor-pointer pb-3">
              ...
              />
              <span className="text-sm text-ink-2">{t('v2Common.autoPauseExpired')}</span>
            </label>
          </div>
            ...
            <div className="grid grid-cols-2 gap-3 pt-3 border-t border-line-1">
              ...
              <label className="inline-flex items-center gap-2 cursor-pointer pb-3 self-end">
                <input
                  ...
                  className="w-4 h-4 accent-orange"
                />
                <span className="text-sm text-ink-2">{t('v2Common.schedulable')} ({t('v2Common.schedulableHint')})</span>
              </label>
            </div>
          )}
        </form>
